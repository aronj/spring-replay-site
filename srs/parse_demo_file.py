...
class Parse_demo_file():
    ...
    def check_magic(self):
        ...
        with myopen(self.filename, "rb") as demofile:
            demofile.seek(0)
            _magic = demofile.read(15).decode()
            if _magic != 'spring demofile':
    ...
    def parse_header(self):
        ...
        #
        self.header['magic'] = self.read_blob(0, 15).decode()  # char magic[16]; ///< DEMOFILE_MAGIC
        if self.header['magic'] != 'spring demofile':
        ...
        for k, v in self.header.items():
            if k == 'gameID':
                self.header['gameID'] = ("%02x" * 16) % unpack("16B", v)
            elif k == 'unixTime':
                self.header['unixTime'] = unpack("Q", self.header['unixTime'])[0]
            else:
                if type(v) == bytes:
                    self.header[k] = v.decode()
                else:
                    self.header[k] = v

        self.header['versionString'] = self.header['versionString'].replace('\x00', '')
        self.header['gameTime'] = "%s" % timedelta(seconds=self.header['gameTime'])
        self.header['wallclockTime'] = "%s" % timedelta(seconds=self.header['wallclockTime'])
    ...
    def parse_script(self):
        script = self.read_blob(self.header['headerSize'], self.header['scriptSize']).decode()
        ...
        # for k, v in self.game_setup.items():
        #     print(type(v))
        #     if type(v) == bytes:
        #         print(k)
        #         print(v.decode())
    ...
    def parse_demostream(self):
        ...
        self.options = {
            **_dictify(self.script.modoptions),
            **_dictify(self.script.other),
            **_dictify(self.script.mapoptions)
        }
        ...
        while packet:
            ...
            try:
                ...
                def clean(name):
                    ...
                    # if isinstance(name, bytes) and len(name) > 0:
                    #     name = name.replace(b'\x00', b'').decode()
                    # else:
                    #     name = ''
                    return name.decode().replace('\x00', '') if type(name) is bytes else name
                ...
                if messageData:
                    ...
                    if messageData['cmd'] == 'keyframe':
                        ...
                        if messageData['action'] == 'team_died':  # team died event
                            ...
                            self.dead_teams.append(deadTeam)
                        ...
                        elif messageData['action'] == 'ai_destroyed':
                            pprint.pprint(messageData)
                            set_trace()
                    ...
                    # elif messageData["cmd"] == "chat":
                        # self.additional["chat"].append({"fromID": clean(messageData["fromID"]),
                        #                                 "playerName": clean(messageData["playerName"]),
                        #                                 "toID": clean(messageData["toID"]),
                        #                                 # "message": clean(messageData["message"][:-1])
                        #                                 "message": clean(messageData["message"])
                        #                                 })
                            ...
                            try:
                                ...
                                for pos in range(1, len(str_B)):
                                    if int(str_B[pos]) > 160:
                                        ...
                                        s_ = "".join([x.decode() for x in str_c[start + 1:end]]).split(":")
                                        try:
                                    ...
                                    awards_data.append(str_B[start])
                                    s_ = "".join([x.decode() for x in str_c[start + 1:]]).split(":")
                                    try:
                                        awards_data.append([int(s_[0]) - 1, f_(s_[1])])
                                    except IndexError:
                                        # gadget version < r2043 has no ':score' after the teamID
